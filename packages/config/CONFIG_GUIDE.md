...
### Era Bounds (Staking Monitor)

For the Staking monitor, you can specify era validity bounds at the group level (via monitor settings) or account level:

- `fromEra` (optional): Era from which to start monitoring (inclusive)
- `untilEra` (optional): Era before which to stop monitoring (exclusive)
- Either or both can be omitted for open-ended monitoring

These bounds apply to all state handlers for that account. This is useful during nomination changes to prevent false incidents when transitioning between validator cohorts.
...
groups:
  - name: validators-cohort-1-old
    chains: [Polkadot]
    accountSet: accounts-cohort-1
    monitors:
      - name: Staking
        untilEra: 1050  # Monitor cohort 1 until era 1050 (not including)
        handlers:
          - ActiveSetPresenceState

  - name: validators-cohort-2-new
    chains: [Polkadot]
    accountSet: accounts-cohort-2
    monitors:
      - name: Staking
        fromEra: 1050  # Start monitoring cohort 2 from era 1050
        handlers:
          - ActiveSetPresenceState
```

Era bounds can also be specified at the account level to override group settings:
