...
export class Migration1755171581759 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."notification_type_enum" AS ENUM('Alert', 'Resolution', 'Escalation')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `UPDATE "notification" SET "repeat_firing_ms" = CAST("repeat_hours_temp" * 3600000 AS integer) WHERE "repeat_hours_temp" IS NOT NULL`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE "notification" SET "repeat_hours" = CAST("repeat_firing_ms_temp" / 3600000.0 AS double precision) WHERE "repeat_firing_ms_temp" IS NOT NULL`,
    );
