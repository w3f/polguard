...
export class XcmMonitor extends AbstractChainMonitor<MonitorType.Xcm> {
  @Event(
    H.XcmTransferEgress,
  ...
  async xcmTransferEgress({
    ...
    blockNumber,
    handler,
  }: EventHandlerParams<H.XcmTransferEgress>): Promise<IncidentPayload[]> {
    let transferInfo;
    ...
    for (const { account, alerts, groupId } of this.getAccounts(handler, origin)) {
      const message = this.createMessage(
        [
          `${this.formatAccountLink(account)} sent ${this.formatBalance(amount)} ${token || this.chainProps.chainToken}`,
