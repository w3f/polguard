...
export class IdentityMonitor extends AbstractChainMonitor<MonitorType.Identity> {
  ...
  async identityUnexpected({
    blockNumber,
    handler,
  }: StateHandlerParams<H.IdentityUnexpected>): Promise<IncidentPayload[]> {
  ...
  async identityMissing({ blockNumber, handler }: StateHandlerParams<H.IdentityMissing>): Promise<IncidentPayload[]> {
    const addressToParent = await this.getAddressToParent(blockNumber);
    const parents = Array.from(new Set(addressToParent.values()));
    const identities = await this.provider.identityOf(parents, blockNumber);
    const incidents: IncidentPayload[] = [];
  ...
  async identityFieldsMissing({
    blockNumber,
    handler,
  }: StateHandlerParams<H.IdentityFieldsMissing>): Promise<IncidentPayload[]> {
