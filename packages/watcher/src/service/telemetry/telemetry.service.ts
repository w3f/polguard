...
export class TelemetryService implements TelemetryClient {
  ...
  constructor(
    ...
    if (!telemetryConfig) {
      throw new Error('Telemetry configuration is required when watcherType is Telemetry');
    }

    this.endpoint = telemetryConfig.endpoint;
    this.username = telemetryConfig.basicAuth?.username;
    this.password = telemetryConfig.basicAuth?.password || process.env.TELEMETRY_PASSWORD;
  }
